...
export const ProfilePage = () => {
    ...
    const convertFriendID = (friendID) => {
        if (friendID.length == 0) {
            var newFriendData = ''
            fetch((process.env.BACKEND_URL + `/getuserconvert/${friendID}`))
                .then(res => res.json())
                .then(data => {
                    newFriendData = data
                })
                            ...
                            <div className="dropdown">
                                ...
                                <ul className="dropdown-menu">
                                    {store.friendRequests.map((user) => {
                                        return (
                                            <li key={user.user.uid}><span className="dropdown-item" href="#">
                                                <span>{user.user.username}</span>
                                                <i className="fas fa-check" style={{ color: "#30810e" }}></i>
                                                <i className="fas fa-window-close" style={{ color: "#f50505" }}></i>
                                            </span></li>
                                        )
                                    })}
                            ...
                            <table className="table table-dark blurbg">
                                ...
                                <tbody>
                                    {store.friends.map((friend, ind) => {
                                        return <tr key={ind} onClick={() => { navigate(`/profile/${data.friend.user.username}`, { state: { userData: friend.user } }) }}>
